...
var wobbleSpeed = 7, wobbleDist = 0.08;
...
var wobbleSpeed = 9, wobbleDist = 0.08;
...
Player.prototype.act = function(step, level, keys) {
  ...
  if (level.status == "lost") {
    this.pos.y += step;
    this.size.y -= step;
  } else if (level.status == "portal") {
	  this.originalH = this.pos.y;
	 level.status = "teleporting";
	 // this.pos.y -= .5;
	  //this.speed = .5;
	//if (this.pos.y < 3) {
	//	level.status = null;
  ...
  else if (level.status == "teleporting") {
		this.pos.y -= .5;
		
		if (this.pos.y < (this.originalH - 15)) {
			level.status = null;
		}
	} 
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
   ...
   //player.style.background = rgb(169, 12, 242);
   console.log("Super");
   //document.getElementById("player")= Player2;
   //this.size = new Vector(10, 3);
